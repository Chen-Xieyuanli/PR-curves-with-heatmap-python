...
def calcPRC(score,target,args,resultsString):
    ...
    for thres in thresValsList:
        ...
        diffPrecision = abs(workingPrecision - prevPrecision)
        diffRecall = abs(workingRecall - prevRecall)
        diffThres = abs(float(prevThreshold)-float(thres))
        if diffPrecision < 0.00002 and diffRecall < 0.00001 and diffThres< 0.055 :
            smallChanges = smallChanges + 1
        elif( diffPrecision < 0.00001 and diffRecall < 0.000008 ):
            smallChanges = smallChanges + 1
        prevRecall = workingRecall
        prevPrecision = workingPrecision
        prevThreshold = thres
