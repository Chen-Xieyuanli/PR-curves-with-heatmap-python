...
def calcAuc(detectionPaths,groundtruthPath):
    ...
    for detections in detectionPaths:
        ...
        modelDsX = modelDs[0]
        modelDsY = modelDs[1]
        print "ModelDsX: %s ModelDsY: %s" % (modelDsX,modelDsY)
...
def findAllAucResultFiles():
    ...
    for dirname, dirnames, filenames in os.walk('.'):
        for filename in filenames:
            if( "results" in filename and "detector-mDs[" in filename):
                #print filename
...
def genereateHeatMap():
    ...
    for line in inputFile:
        ...
        data[ (modelDsX) , (modelDsY) ] = foundAUC
    ...
    plt.xlabel('Model Dimension X-Size')
    plt.ylabel('Model Dimension Y-Size')
    plt.title('Heatmap of Model Dimension for ACF Detector')
